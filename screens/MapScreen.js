...
export default function MapScreen({ navigation }) {
  ...
  const syncMarkers = () => {
    const filtered = applyFilters(pgs);
    postToWeb({ type: 'renderMarkers', payload: filtered });
    postToWeb({ type: 'fitToMarkers' });
  };
  ...
  useEffect(() => {
    syncMarkers();
  }, [pgs, filterLocation, filterMaxPrice]);
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={styles.container}>
        <View style={styles.controls}>
          <Text style={styles.header}>Find PGs</Text>
          <View style={styles.row}>
            <TextInput style={styles.input} placeholder="Location contains" value={filterLocation} onChangeText={setFilterLocation} />
            <TextInput style={styles.input} placeholder="Max Price" value={filterMaxPrice} onChangeText={setFilterMaxPrice} keyboardType="numeric" />
          </View>
          <TouchableOpacity style={styles.primaryBtn} onPress={syncMarkers}>
        ...
        <TouchableOpacity style={styles.fab} onPress={goToAddPg} activeOpacity={0.85}>
